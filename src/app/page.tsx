  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="border-b border-slate-100 py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center mb-16">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-success mb-6">Institutional Pillars</p>
            <h2 className="text-4xl md:text-5xl font-black italic tracking-tighter text-slate-950 mb-4">
              UNIFIED <span className="text-success">LOGIC</span> ECOSYSTEM.
            </h2>
          </div>
      ...
      <section className="bg-slate-50 py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center mb-16">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-success mb-6">Precision Verticals</p>
            <h2 className="text-4xl md:text-5xl font-black italic tracking-tighter text-slate-950">
              INSTITUTIONAL <span className="text-success">GRADE</span> PORTALS.
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {portalTypes.map((portal) => (
              <div key={portal.title} className="bg-white p-10 rounded-[2rem] border border-slate-100 hover:shadow-xl transition-all group">
                ...
                <Link href="/contact" className="inline-flex items-center gap-2 text-success font-black text-[10px] uppercase tracking-widest hover:gap-3 transition-all">
                  REVIEW ARCHITECTURE →
                </Link>
              </div>
            ))}
      ...
      <section className="bg-slate-950 text-white py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center mb-16">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-success mb-6">The Vishray Advantage</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {advantages.map((adv) => (
      ...
      <section className="bg-white py-20 lg:py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-success mb-6">Deploy Vishray</p>
              <h2 className="text-4xl md:text-5xl font-black italic text-slate-950 mb-6 tracking-tighter">
                बुक करें उप्ना <span className="text-success">फ्री</span> डेमा.
              </h2>
              ...
              <div className="flex items-center gap-4">
                <button onClick={() => setIsDemoOpen(true)} className="px-8 py-4 bg-success text-white rounded-2xl font-black uppercase tracking-widest text-sm hover:scale-105 transition-all shadow-lg shadow-success/20">
                  INITIATE AUDIT
                </button>
              </div>
              <div className="mt-10 flex items-center gap-4">
                <div className="flex -space-x-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="w-10 h-10 bg-success/20 rounded-full border-2 border-white"></div>
                  ))}
                </div>
            ...
            <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
              <div className="space-y-6">
                <div>
                  ...
                  <input type="text" placeholder="Agent Name" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-success focus:outline-none font-medium" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Secure Link</label>
                  <input type="tel" placeholder="Phone Number" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-success focus:outline-none font-medium" />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-2">Sector Logic</label>
                  <select className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-success focus:outline-none font-medium">
                ...
                <button onClick={() => setIsDemoOpen(true)} className="w-full px-6 py-4 bg-success text-white rounded-2xl font-black uppercase text-sm hover:scale-105 transition-all shadow-lg shadow-success/20">
                  INITIATE AUDIT
                </button>
              </div>
            </div>
