...
export default function WebDevPage() {
  ...
  const features = [
    { title: 'SEO Optimized', icon: <Search className="w-8 h-8 text-emerald-500" /> },
    { title: 'Lightning Speed', icon: <Zap className="w-8 h-8 text-emerald-500" /> },
    { title: 'Analytics Integration', icon: <BarChart className="w-8 h-8 text-emerald-500" /> },
    { title: 'Scalable Architecture', icon: <Rocket className="w-8 h-8 text-emerald-500" /> }
  ];
            ...
            <div className="flex flex-col lg:flex-row items-center gap-16">
               <div className="flex-1 space-y-8 w-full">
                  ...
                  <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold text-slate-950 tracking-tight leading-[0.9] italic">
                     Architectural <span className="text-emerald-600">Web Logic.</span>
                  </h1>
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((f, i) => (
              <div key={i} className="group p-10 bg-slate-50 rounded-[3rem] border border-slate-100 hover:border-emerald-200 hover:bg-white hover:shadow-xl transition-all text-center">
                 ...
                 <p className="text-slate-600 font-medium leading-relaxed italic">Industry leading standards followed for every project.</p>
              </div>
            ))}
         </div>
      </section>
