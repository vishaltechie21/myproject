  ...
  return (
    <div ref={containerRef} className="bg-white pb-32">
      ...
      <section className="bg-slate-900 py-32 relative overflow-hidden">
         <div className="max-w-7xl mx-auto px-6 relative z-10 hero-content text-center lg:text-left">
            <div className="flex flex-col lg:flex-row items-center gap-16">
               ...
               <div className="flex-1 w-full max-w-lg lg:max-w-none relative aspect-square lg:aspect-video rounded-[3rem] overflow-hidden border-8 border-slate-800 shadow-2xl bg-white p-12">
                  <div className="w-full h-full bg-slate-50 border border-slate-100 rounded-[2rem] flex items-center justify-center">
                     <div className="text-slate-200 text-5xl font-black rotate-[-15deg] uppercase tracking-tighter opacity-10">Premium UX Mockup</div>
                  </div>
               </div>
            </div>
         </div>
      ...
      <section className="py-32 max-w-7xl mx-auto px-6 text-center">
         <h2 className="text-3xl font-black text-slate-900 mb-16 underline decoration-yellow-400 decoration-4 underline-offset-8">Our Creative Methodology</h2>
         <div className="grid md:grid-cols-4 gap-8">
            {features.map((f, i) => (
              <div key={i} className="p-8 bg-white border border-slate-100 rounded-3xl hover:bg-yellow-50 hover:shadow-xl transition-all h-full scale-[0.98] hover:scale-100 duration-500">
                 <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center mx-auto mb-6 text-yellow-600 shadow-lg border border-yellow-50">{f.icon}</div>
                 <h4 className="text-xl font-black text-slate-900 mb-4">{f.title}</h4>
                 <p className="text-slate-500 font-medium italic text-xs">Combining beauty with functionality for high-impact software.</p>
              </div>
            ))}
         </div>
      </section>
