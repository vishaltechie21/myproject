...
const Footer = () => {
  ...
  return (
    <footer className="bg-slate-50 border-t border-success/10">
            ...
            <div className="flex space-x-4 pt-2">
              {socialLinks.map((social, index) => (
                <a
                  key={index}
                  href={social.href}
                  className="w-8 h-8 flex items-center justify-center rounded-full bg-success/10 text-success hover:bg-success hover:text-white transition-all transform hover:scale-110"
                >
                  {social.icon}
                </a>
              ))}
              ...
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-slate-600 hover:text-success transition-colors text-sm"
                    >
                      {link.label}
                    </Link>
                  </li>
